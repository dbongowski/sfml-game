...
namespace Bongo
{
	TextPenguin::TextPenguin(int size, int UsedIndex, int PlayerNumber, string PlayerName, GameDataRef data) : _data(data)
	{
		this->_data->assets.LoadFont("Question Font", FONT_DROGOWSKAZ_FILEPATH);
		...
		for (int i = 0; i < 2; i++)
		{
			...
			Correct[i].setOutlineThickness(3);
			Correct[i].setOutlineColor(sf::Color::Black);
		}
		...
		for (int i = 0; i < 2; i++)
		{
			...
			PlayerData[i].setFillColor(sf::Color::Black);
		}
	...
	void Bongo::TextPenguin::LoadText()
	{
		int i = 0;
		string line;
		ifstream firstFileout, secondFileout;

		firstFileout.open(FIRST_PENGUIN_TXT_FILEPATH);
		while (getline(firstFileout, line))
		{
			First[i] = line;
			First[i] = TextWrapping(First[i], 11);
			i++;
		}

		secondFileout.open(SECOND_PENGUIN_TXT_FILEPATH);
		i = 0;
		while (getline(secondFileout, line))
		{
			Second[i] = line;
			Second[i] = TextWrapping(Second[i], 11);
			i++;
		}
	}

	void Bongo::TextPenguin::texttest()
	{
	...
	void Bongo::TextPenguin::AssignText()
	{
	...
	void Bongo::TextPenguin::Shuffle()
	{
		//Shuffling blocks
	...
	string TextPenguin::TextWrapping(string str, int length)
	{
	...
	void TextPenguin::SaveUsed()
	{

		ofstream myfile(USED_PENGUIN_TXT_FILEPATH);
		if (myfile.is_open())
		{
			for (int i = 0; i < NUMBER_OF_QUESTIONS; i++) {
		...
		ofstream myfile1(USED_INDEX_PENGUIN_TXT_FILEPATH);
		if (myfile1.is_open())
		{
			myfile1 << UsedIndex;
	...
	void TextPenguin::LoadUsed()
	{

		ifstream File;
		File.open(USED_PENGUIN_TXT_FILEPATH);
		...
		ifstream File1;
		File1.open(USED_INDEX_PENGUIN_TXT_FILEPATH);
	...
	void TextPenguin::SaveBlockNums()
	{
	...
	void TextPenguin::ClearBlockNums()
	{
		ofstream myfile(BLOCK_NUMS_TXT_FILEPATH);
		if (myfile.is_open())
		{
	...
	int TextPenguin::ReturnUsedIndex()
	{
		return UsedIndex;
	}


	void TextPenguin::DrawText()
	{
		for (int i = 0; i < NUMBER_OF_BLOCKS; i++)
		{
	...
	void TextPenguin::DrawCorrect(bool correct)
	{
		if (correct == true)
		{
	...
	void TextPenguin::DrawHowTo()
	{
		this->_data->window.draw(this->PressF1);
	}
	void TextPenguin::DrawEND()
	{

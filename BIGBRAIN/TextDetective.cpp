...
namespace Bongo
{
	TextDetective::TextDetective(int size, int UsedIndex, int PlayerNumber, string PlayerName, GameDataRef data) : _data(data)
	{
		this->_data->assets.LoadFont("Question Font", FONT_BERKSHIRE_FILEPATH);
	...
	void Bongo::TextDetective::LoadText()
	{
		int i = 0;
		string line;
		ifstream firstFileout, secondFileout;

		firstFileout.open(FIRST_DETECTIVE_TXT_FILEPATH);
		...
		secondFileout.open(SECOND_DETECTIVE_TXT_FILEPATH);
		i = 0;
	...
	void Bongo::TextDetective::texttest()
	{
	...
	void Bongo::TextDetective::AssignText()
	{
	...
	void Bongo::TextDetective::Shuffle()
	{
		//Shuffling blocks
	...
	string TextDetective::TextWrapping(string str, int length)
	{
	...
	void TextDetective::SaveUsed()
	{

		ofstream myfile(USED_DETECTIVE_TXT_FILEPATH);
		if (myfile.is_open())
		{
			for (int i = 0; i < NUMBER_OF_QUESTIONS; i++) {
		...
		ofstream myfile1(USED_INDEX_DETECTIVE_TXT_FILEPATH);
		if (myfile1.is_open())
		{
			myfile1 << UsedIndex;
	...
	void TextDetective::LoadUsed()
	{

		ifstream File;
		File.open(USED_DETECTIVE_TXT_FILEPATH);
		...
		ifstream File1;
		File1.open(USED_INDEX_DETECTIVE_TXT_FILEPATH);
	...
	void TextDetective::UsedRestart()
	{
		ofstream myfile(USED_DETECTIVE_TXT_FILEPATH);
		if (myfile.is_open())
		{
	...
	void TextDetective::SaveBlockNums()
	{
	...
	void TextDetective::ClearBlockNums()
	{
		ofstream myfile(BLOCK_NUMS_TXT_FILEPATH);
		if (myfile.is_open())
		{
	...
	void TextDetective::SavePairNums()
	{
	...
	int TextDetective::ReturnUsedIndex()
	{
		return UsedIndex;
	}


	void TextDetective::DrawText()
	{
		for (int i = 0; i < NUMBER_OF_BLOCKS; i++)
		{
	...
	void TextDetective::DrawCorrect(bool correct)
	{
		if (correct == true)
		{
	...
	void TextDetective::DrawHowTo()
	{
		this->_data->window.draw(this->PressF1);
	}
	void TextDetective::DrawEND()
	{

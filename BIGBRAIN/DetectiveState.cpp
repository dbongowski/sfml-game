...
Bongo::DetectiveState::DetectiveState(GameDataRef data) : _data(data)
{
}

void Bongo::DetectiveState::Init()
{
	this->_data->assets.LoadTexture("Floor Detective Texture", FLOOR_DETECTIVE_TEXTURE_FILEPATH);
	this->_data->assets.LoadTexture("Block Texture", BLOCK_STONE_TEXTURE_FILEPATH);
	this->_data->assets.LoadTexture("Detective Texture", DETECTIVE_TEXTURE_FILEPATH);
	this->_data->assets.LoadTexture("Detective Background", DETECTIVE_BACKGROUND_FILEPATH);
	...
	Background.setTexture(this->_data->assets.GetTexture("Detective Background"));

	LoadProfiles();

	exitsave = new ExitSave(_data);
	floor = new StartBackground(&this->_data->assets.GetTexture("Floor Detective Texture"), sf::Vector2f(800.0f, 100.0f), sf::Vector2f(360.0f, -30.0f), sf::Vector2f(360.0f, 480.0f), _data);
	wallL = new StartBackground(nullptr, sf::Vector2f(100.0f, 700.0f), sf::Vector2f(0.0f, 0.0f), sf::Vector2f(-100.0f, 0.0f), _data);
	wallR = new StartBackground(nullptr, sf::Vector2f(100.0f, 700.0f), sf::Vector2f(0.0f, 0.0f), sf::Vector2f(820.0f, 0.0f), _data);
	block1 = new Block(&this->_data->assets.GetTexture("Block Texture"), sf::Vector2f(175.0f, 100.0f), sf::Vector2f(35.0f, -35.0f), sf::Vector2f(48.0f, 120.0f), _data);
	block2 = new Block(&this->_data->assets.GetTexture("Block Texture"), sf::Vector2f(175.0f, 100.0f), sf::Vector2f(35.0f, -35.0f), sf::Vector2f(248.0f, 120.0f), _data);
	block3 = new Block(&this->_data->assets.GetTexture("Block Texture"), sf::Vector2f(175.0f, 100.0f), sf::Vector2f(35.0f, -35.0f), sf::Vector2f(448.0f, 120.0f), _data);
	block4 = new Block(&this->_data->assets.GetTexture("Block Texture"), sf::Vector2f(175.0f, 100.0f), sf::Vector2f(35.0f, -35.0f), sf::Vector2f(648.0f, 120.0f), _data);
	player = new Player(&this->_data->assets.GetTexture("Detective Texture"), 150.0f, _data);
	text = new TextDetective(22, 0, PlayerNumber, PlayerNames[PlayerNumber], _data);
	F1howto = new F1HowTo(_data);
	LoadBlockNums();
	LoadPairNums();

	PointsFrame.setFillColor(sf::Color::Black);
	PointsFrame.setOutlineColor(sf::Color::White);
	...
	PointsFrame.setPosition(705.0f, 80.0f);

}

void Bongo::DetectiveState::HandleInput()
{
	sf::Event event;
	...
	while (this->_data->window.pollEvent(event))
	{
		...
		{
			text->UsedRestart();
			exitsave->ClearUsed(USED_INDEX_DETECTIVE_TXT_FILEPATH);
			this->_data->machine.AddState(StateRef(new ChooseLevelState(_data)), true);
		}
...
void Bongo::DetectiveState::Update(float dt)
{
		...
		if (block1->GetCollider().CheckCollision(playerCollision, direction, 1.0f))
		{
			...
			if (SelectCount == NUMBER_OF_SELECTIONS)
			{
				...
				text->SaveUsed();
				text->~TextDetective();
				text = new TextDetective(25, UsedIndex, PlayerNumber, PlayerNames[PlayerNumber], _data);
				LoadBlockNums();
				LoadPairNums();
...
void Bongo::DetectiveState::Draw(float dt)
{


	this->_data->window.clear(sf::Color(51, 51, 51));
...
void Bongo::DetectiveState::CheckCorrect()
{
...
void Bongo::DetectiveState::LoadBlockNums()
{
	ifstream File;
...
void Bongo::DetectiveState::LoadPairNums()
{
	ifstream File1;
...
void Bongo::DetectiveState::SavePoints()
{
...
void Bongo::DetectiveState::LoadProfiles()
{
		int n = 0;

...
Bongo::MusicState::MusicState(GameDataRef data) : _data(data)
{
}

void Bongo::MusicState::Init()
{
	this->_data->assets.LoadTexture("Floor Music Texture", FLOOR_MUSIC_TEXTURE_FILEPATH);
	this->_data->assets.LoadTexture("Block Texture", BLOCK_SPEAKER_TEXTURE_FILEPATH);
	this->_data->assets.LoadTexture("Slash Texture", SLASH_TEXTURE_FILEPATH);
	this->_data->assets.LoadTexture("Music Background", MUSIC_BACKGROUND_FILEPATH);
	...
	Background.setTexture(this->_data->assets.GetTexture("Music Background"));

	LoadProfiles();

	exitsave = new ExitSave(_data);
	floor = new StartBackground(&this->_data->assets.GetTexture("Floor Music Texture"), sf::Vector2f(800.0f, 100.0f), sf::Vector2f(360.0f, -30.0f), sf::Vector2f(360.0f, 480.0f), _data);
	...
	player = new Player(&this->_data->assets.GetTexture("Slash Texture"), 150.0f, _data);
	text = new TextMusic(21, 0, PlayerNumber, PlayerNames[PlayerNumber], _data);
...
void Bongo::MusicState::HandleInput()
{
	sf::Event event;
	...
	while (this->_data->window.pollEvent(event))
	{
		...
		{
			text->UsedRestart();
			exitsave->ClearUsed(USED_INDEX_MUSIC_TXT_FILEPATH);
			this->_data->machine.AddState(StateRef(new ChooseLevelState(_data)), true);
		}
...
void Bongo::MusicState::Update(float dt)
{
		...
		if (block1->GetCollider().CheckCollision(playerCollision, direction, 1.0f))
		{
			...
			if (SelectCount == NUMBER_OF_SELECTIONS)
			{
				...
				text->SaveUsed();
				text->~TextMusic();
				text = new TextMusic(25, UsedIndex, PlayerNumber, PlayerNames[PlayerNumber], _data);
				LoadBlockNums();
				LoadPairNums();
...
void Bongo::MusicState::Draw(float dt)
{
...
void Bongo::MusicState::CheckCorrect()
{
...
void Bongo::MusicState::LoadBlockNums()
{
	ifstream File;
...
void Bongo::MusicState::LoadPairNums()
{
	ifstream File1;
...
void Bongo::MusicState::SavePoints()
{
...
void Bongo::MusicState::LoadProfiles()
{
		int n = 0;
